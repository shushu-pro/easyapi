...
export default function ({ http, adapter }, { tests, test, assert }) {
  tests('example', () => {
    test('example.adapter', () => {
      const api = http.create(200, {
        request (config) {
          ...
          if (typeof meta.request === 'function') {
            config.sendData = meta.request(config)
          } else if (typeof meta.request === 'object') {
            config.sendData = adapter(meta.request, config.sendData)
          }
        },
        config: {
          request (config) {
      ...
      return api.test({ name: '张三' }).then((data) => {
      //  console.info({ data })
      })
    })
  })
