...
export default {
  ...
  resolve (thisRuntime) {
    if (resolve) {
      ...
      console.warn(
        `${indent(-1)}${matcherIdentfier(matcher)}${matcherInfo(matcher)} resolve ${matchedString(thisRuntime)}`,
        [ matcher ],
      )
    }
  },
  reject (thisRuntime, error) {
    if (reject) {
      ...
      console.error(
        `${indent(-1)}${matcherIdentfier(matcher)}${matcherInfo(matcher)} reject ${chNow(sr)}`,
        [ matcher, error ],
      )
    }
  },
